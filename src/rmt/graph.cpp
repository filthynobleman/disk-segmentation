...
Graph::Graph(const Eigen::MatrixXd& V, const std::set<std::pair<int, int>>& E)
{
    ...
    for (auto e : E)
    {
        Edges.emplace(std::min(e.first, e.second), std::max(e.first, e.second));
        Edges.emplace(std::max(e.first, e.second), std::min(e.first, e.second));
    }
...
Eigen::VectorXd rmt::Graph::DijkstraDistance(int src, const Eigen::VectorXi &Tag, int Filter) const
{
    Eigen::VectorXd Dists;
    Dists.setConstant(NumVertices(), std::numeric_limits<double>::infinity());
...
Eigen::VectorXi Graph::ConnectedComponents(const std::unordered_set<std::pair<int, int>, rmt::PairHash<int>>& Ignore) const
{
    Eigen::VectorXi CC;
    CC.setZero(NumVertices());
    int CurCC = 0;
    ...
    while (Visited.sum() < NumVertices())
    {
        ...
        while (!Q.empty())
        {
            ...
            {
                int j = GetAdjacent(N, jj).first;
                // Make edge, canonically ordered
                std::pair<int, int> e{ N, j };
                if (e.first > e.second)
                    std::swap(e.first, e.second);
                // Traverse edge only if not to ignore
